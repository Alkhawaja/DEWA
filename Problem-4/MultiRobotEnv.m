classdef MultiRobotEnv < matlab.System
    ...
    methods (Access = public) 
        ...
        function setupVisualization(obj,poses)
            ...
            obj.trajY = cell(obj.numRobots,1);
                ...
                if obj.showTrajectory(rIdx)
                                                obj.TrajHandle{rIdx} = plot(obj.ax,0,0,'x','color',[1 1 1]);%[abs(30-cost(obj.trajX{rIdx},obj.trajY{rIdx}))/30                        abs(30-cost(obj.trajX{rIdx},obj.trajY{rIdx}))/30 0]);
                    obj.trajX{rIdx} = [];
                    obj.trajY{rIdx} = [];
                end
            end
        ...
        function drawRobot(obj,rIdx,pose,ranges,cost)
            ...
            hold on
            %  if cost(ceil(x+ii),ceil(y+kk))>30
      %  plot(ceil(x+ii),ceil(y+kk),'xr')
           %     else
           if(x>0&&x<100&&y>0&&y<100)
            plot(x+ii,y+kk,'x','color',[abs(200-cost(ceil(x+ii),ceil(y+kk)))/200 abs(200-cost(ceil(x+ii),ceil(y+kk)))/200 0] );
                end
                        end
            end
...
%                     
%             hold on
%             plot(x+ii,y+kk,'x','color',[abs(100-cost(ceil(x+ii),ceil(y+kk)))/200 abs(100-cost(ceil(x+ii),ceil(y+kk)))/200 0] );
%                 end
%             end
